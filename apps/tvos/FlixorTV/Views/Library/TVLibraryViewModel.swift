//
//  TVLibraryViewModel.swift
//  FlixorTV
//
//  View model powering the Library / Browse experience for tvOS.
//
...
final class TVLibraryViewModel: ObservableObject {
    ...
    func loadIfNeeded(preferredKind: LibrarySectionSummary.Kind? = nil) async {
        guard !sectionsLoaded else { return }
        do {
            try await fetchSections()
            sectionsLoaded = true

            // Auto-select section based on preferred kind (from tab navigation)
            if let kind = preferredKind {
                if let section = sections.first(where: { $0.kind == kind }) {
                    activeSection = section
                } else if let first = sections.first {
                    activeSection = first
                }
            } else if activeSection == nil, let first = sections.first {
                activeSection = first
            }
        } catch {
    ...
    private func applyFilters() {
        ...
        output.sort { lhs, rhs in
            ...
            case .dateAddedDesc, .dateAddedAsc:
                // API handles date sorting, keep current order
                return false
            case .titleAsc:
