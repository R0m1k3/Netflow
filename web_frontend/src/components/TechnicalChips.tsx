...
export default function TechnicalChips({ info }: { info: MediaInfo }) {
  ...
  if (info.bitrateKbps) chips.push(`${Math.round(info.bitrateKbps / 1000)} ${t('player.mbps')}`);
  if (info.fileSizeMB) chips.push(`${Math.round(info.fileSizeMB)} MB`);
  if (info.subsCount) chips.push(`${info.subsCount} ${t('details.subtitles').toLowerCase()}`);
  return (
