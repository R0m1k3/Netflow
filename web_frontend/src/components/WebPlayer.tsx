...
export default function WebPlayer({
  ...
  useEffect(() => {
    ...
    v.addEventListener('error', () => setError(t('player.error_browser')));
    ...
    <div ref={containerRef} className="fixed inset-0 bg-black" onClick={onBackdropClick}>
      ...
      {error && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="rounded-xl ring-1 ring-white/10 bg-black/70 p-4 text-center text-neutral-200">
            <div className="mb-2">{t('player.error_browser')}</div>
            <div className="flex gap-2 justify-center">
              <button className="btn" onClick={() => window.open(src, '_blank')}>{t('player.open_tab')}</button>
              <button className="btn" onClick={() => { setError(null); const v = videoRef.current; v?.load(); v?.play().catch(() => { }); }}>{t('player.retry')}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className={`absolute top-0 inset-x-0 p-3 transition-opacity ${showUi ? 'opacity-100' : 'opacity-0'} bg-gradient-to-b from-black/60 to-transparent`}>
        <div className="max-w-6xl mx-auto flex items-center gap-3 text-neutral-200">
          <button className="px-2 py-1 rounded bg-white/10 hover:bg-white/20" onClick={onBack}>{t('player.back')}</button>
      ...
      <div className={`absolute inset-0 flex items-center justify-center pointer-events-none transition-opacity ${showUi ? 'opacity-100' : 'opacity-0'}`}>
        <button className="pointer-events-auto px-4 py-2 rounded-full bg-white/15 hover:bg-white/25 text-white" onClick={togglePlay}>{playing ? t('player.pause') : t('player.play')}</button>
      </div>
      {/* Skip intro */}
      {activeSkip && (
        <div className="absolute bottom-24 right-6">
          <button className="px-3 py-1.5 rounded-full bg-white/15 hover:bg-white/25 text-white text-sm" onClick={() => { const v = videoRef.current; if (!v) return; v.currentTime = Math.min(v.duration, activeSkip.end + 0.5); }}>{t('player.skip_intro')}</button>
        </div>
      )}
      {/* Bottom controls */}
      <div className={`absolute bottom-0 inset-x-0 p-4 transition-opacity ${showUi ? 'opacity-100' : 'opacity-0'} bg-gradient-to-t from-black/70 to-transparent player-controls`}>
        <div className="max-w-6xl mx-auto space-y-2">
          ...
          <div className="flex items-center gap-3 text-sm text-neutral-200">
            ...
            <button className="px-2 py-1 rounded bg-white/10 hover:bg-white/20" onClick={togglePlay}>{playing ? t('player.pause') : t('player.play')}</button>
            ...
            <div className="flex items-center gap-2 ml-4">
              <button
                ...
                title={volume > 0 ? t('details.mute') : t('details.unmute')}
              >
            ...
            <div className="flex items-center gap-2 ml-2">
              <span className="text-xs text-neutral-300">{t('player.speed')}</span>
            ...
            {qualityOptions && qualityOptions.length > 0 && (
              <div className="flex items-center gap-2 ml-2">
                <span className="text-xs text-neutral-300">{t('player.quality')}</span>
            ...
            {resolutionOptions && resolutionOptions.length > 0 && (
              <div className="flex items-center gap-2 ml-2">
                <span className="text-xs text-neutral-300">{t('player.resolution')}</span>
            ...
            {pipSupported && (
              <button className="px-2 py-1 rounded bg-white/10 hover:bg-white/20" onClick={() => togglePiP()} title={t('player.pip')}>
                {pipActive ? t('player.pip_on') : t('player.pip')}
              </button>
            )}
            <button className="px-2 py-1 rounded bg-white/10 hover:bg-white/20" onClick={toggleFullscreen}>{t('player.full')}</button>
          </div>
        </div>
      </div>
      {/* Next episode prompt */}
      {nextLabel && (nearEnd || showUi) && (
        <div className="absolute bottom-24 right-6">
          <div className="rounded-xl bg-black/60 ring-1 ring-white/10 p-3 text-neutral-200">
            <div className="text-sm mb-2">{t('player.up_next')} {countdown !== null && <span className="text-neutral-400">• {t('player.playing_in', { count: countdown })}</span>}</div>
            <div className="font-medium mb-2 max-w-xs line-clamp-2">{nextLabel}</div>
            <div className="flex justify-end">
              {countdown !== null && (
                <button className="px-3 py-1.5 rounded bg-white/10 hover:bg-white/20 mr-2" onClick={() => { setAutoCanceled(true); setCountdown(null); }}>{t('player.cancel')}</button>
              )}
              <button className="px-3 py-1.5 rounded bg-white/20 hover:bg-white/30" onClick={() => onNext?.()}>{t('player.play_now')}</button>
            </div>
          </div>
        </div>
      )}
