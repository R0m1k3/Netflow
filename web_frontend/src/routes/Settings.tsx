  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-3xl mx-auto p-6 space-y-8 pt-6">
        <section>
          <h2 className="text-xl font-semibold mb-2">{t('settings.accounts')}</h2>
          <div className="grid gap-3">
            {/* Plex authentication via PIN */}
            <div className="rounded-lg ring-1 ring-white/10 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm text-neutral-300">{t('settings.plex_account')}</div>
                  {selectedServer ? (
                    <div className="text-neutral-200 text-sm">{t('settings.connected_to')} {selectedServer?.name}</div>
                  ) : initial.plexAccountToken ? (
                    <div className="text-neutral-400 text-sm">{t('settings.authenticated_select')}</div>
                  ) : (
                    <div className="text-neutral-400 text-sm">{t('settings.not_signed_in')}</div>
                  )}
                </div>
                {!initial.plexAccountToken && !auth.pinId && (
                  ...
                  }}>{t('settings.sign_in_plex')}</button>
                )}
                {initial.plexAccountToken && (
                  ...
                  }}>{t('settings.refresh_servers')}</button>
                )}
              </div>
              {auth.pinId && (
                <div className="mt-3 text-sm">
                  <div className="mb-2">{t('settings.enter_code')} <span className="font-semibold text-white">{auth.code}</span></div>
                  <div className="flex gap-2">
                    <button className="btn" onClick={() => window.open((auth as any).url || '', '_blank')}>{t('settings.open_plex')}</button>
                    <button className="btn" onClick={async () => {
                    ...
                    }}>{t('settings.poll')}</button>
                  </div>
                </div>
              )}
              {servers.length > 0 && (
                <div className="mt-3">
                  <div className="text-sm mb-2">{t('settings.select_server')}</div>
            ...
            <L label={t('settings.plex_url')}><input value={plexUrl} onChange={(e) => setPlexUrl(e.target.value)} placeholder="https://app.plex.tv" className="input" /></L>
            <L label={t('settings.plex_token')}><input value={plexToken} onChange={(e) => setPlexToken(e.target.value)} placeholder="" className="input" /></L>
            <L label={t('settings.tmdb_key')}><input value={tmdbKey} onChange={(e) => setTmdbKey(e.target.value)} className="input" /></L>
            <L label={t('settings.watchlist_provider')}>
              <select
                className="input"
                ...
                onChange={(e) => {
                  ...
                  try { window.dispatchEvent(new CustomEvent('app-toast', { detail: `${t('settings.watchlist_provider')}: ${v === 'trakt' ? 'Trakt' : 'Plex'}` })); } catch { }
                }}
              >
              ...
              {watchlistProvider === 'trakt' && !traktConnected && (
                <div className="text-xs text-yellow-300 mt-1">{t('settings.trakt_not_connected')}</div>
              )}
            </L>
            <div className="flex gap-3 pt-2">
              <button className="btn" onClick={async () => {
                setTmdbStatus(t('common.testing'));
                try {
                  const { tmdbTrending } = await import('@/services/tmdb');
                  await tmdbTrending(tmdbKey, 'tv', 'day');
                  setTmdbStatus(`TMDB ${t('common.ok')}`);
                } catch (e: any) {
                  setTmdbStatus(`TMDB ${t('common.failed')} ` + (e?.message || 'error'));
                }
              }}>{t('settings.test_tmdb')}</button>
              <span className="text-sm text-neutral-400">{tmdbStatus}</span>
            </div>
            <div className="flex gap-3">
              <button className="btn" onClick={async () => {
                setPlexStatus(t('common.testing'));
                try {
                  const { plexLibs } = await import('@/services/plex');
                  await plexLibs({ baseUrl: plexUrl, token: plexToken });
                  setPlexStatus(`Plex ${t('common.ok')}`);
                } catch (e: any) {
                  setPlexStatus(`Plex ${t('common.failed')} ` + (e?.message || 'error'));
                }
              }}>{t('settings.test_plex')}</button>
              <span className="text-sm text-neutral-400">{plexStatus}</span>
            </div>
            <div className="flex gap-3">
              <button className="btn" onClick={() => { forget(''); alert('Cache cleared'); }}>{t('settings.clear_cache')}</button>
              <button className="btn" onClick={() => { saveSettings({ plexAccountToken: undefined, plexServer: undefined }); alert('Signed out from Plex'); }}>{t('settings.sign_out_plex')}</button>
            </div>
          </div>
        </section>
        {/* Backend-managed servers section */}
        <section>
          <h2 className="text-xl font-semibold mb-2">{t('settings.backend_servers')}</h2>
          <div className="rounded-lg ring-1 ring-white/10 p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm text-neutral-300">{t('settings.backend_servers_desc')}</div>
              <button className="btn" onClick={async () => {
                try {
                  ...
                  <div key={idx} className="flex items-center justify-between bg-white/5 rounded px-3 py-2">
                    ...
                    <div className="flex gap-2">
                      {!s.isActive && (
                        ...
                        }}>{t('settings.make_current')}</button>
                      )}
                      <button className="btn" onClick={async () => {
                        try {
        ...
        {showEndpointsFor && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
            <div className="bg-neutral-900 rounded-lg p-4 w-full max-w-lg ring-1 ring-white/10">
              <div className="flex items-center justify-between mb-3">
                <div className="text-white font-semibold">{t('settings.select_endpoint')}</div>
        ...
        <section>
          <h2 className="text-xl font-semibold mb-2">{t('settings.playback')}</h2>
          <div className="grid gap-3">
            <L label={t('settings.mpv_profile')}>
              <select className="input">
                <option>Direct Play</option>
                <option>Transcode (H264/AAC)</option>
              </select>
            </L>
            <L label={t('settings.hdr_tone_map')}>
              <select className="input">
                <option>auto</option>
